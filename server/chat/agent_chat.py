        ...
        if stream:
            async for chunk in callback.aiter():
                ...
                if data["status"] == Status.start or data["status"] == Status.complete:
                    ...
                    # combileanswer = "结论：" + data["final_answer"] + "<p></p>" + "理由：" + data["output_str"]
                    combileanswer =data["final_answer"]
                    yield json.dumps({"final_answer": combileanswer}, ensure_ascii=False)
                else:
