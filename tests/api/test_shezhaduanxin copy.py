...
def test_knowledge_chat(query, data):
    """
    This function sends a POST request with a query to the knowledge chat API.
    """
    url = f"{api_base_url}/chat/knowledge_base_chat"
...
def test_llm_chat(query, data):
    """
    This function sends a POST request with a query to the llm chat API.
    """
...
def process_csv_files(folder_path, output_file):
    """
    This function processes all CSV files in the folder, extracts data, and calls the API.
    It computes the "是" response ratio for each CSV file and a total ratio.
    ...
    with open(output_file, 'w', encoding='utf-8') as outfile:
        ...
        for file_name in os.listdir(folder_path):
            ...
            if file_path.endswith('.csv'):  # Ensure it's a CSV file
                yes_count = 0
                row_count = 0
                ...
                with open(file_path, 'r', encoding='utf-8') as csvfile:
                    ...
                    # Randomly select 10000 rows from the CSV file
                    selected_rows = random.sample(reader, 50)

                    # Process each selected row
                    for row in selected_rows:
                        # Since all columns represent the message, join them to form the full message
                        ...
                        if "是" in answer:
                            ...
                            print(yes_count)
                        # else:
                        #     print("ATTENTION!!!!!!!!!!!!!!!!!无法判断")
                        #     print(query)
                        row_count += 1

                # Calculate and save the ratio for the current file
                if row_count > 0:
                    yes_ratio = yes_count / row_count
                    outfile.write(f"File: {file_name}, Total tested: {row_count}, '是': {yes_count}, '是' Ratio: {yes_ratio:.2%}\n")
                    print(f"File: {file_name}, Total tested: {row_count}, '是': {yes_count}, '是' Ratio: {yes_ratio:.2%}")
...
if __name__ == "__main__":
    ...
    folder_path = "/home/ding/LLM/Processed_CSVs"  # Replace with your CSV folder path
    output_file = "llmceshi.txt"  # Output text file to save the results
